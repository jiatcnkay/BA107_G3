...
public class GiftDAO implements GiftDAO_interface {
	...
	private static final String FIND_BY_PK_STMT = "SELECT GIFT_NO,GIFT_NAME,GIFT_PRICE,GIFT_PIC FROM GIFT WHERE GIFT_NO=?";
	private static final String GET_ALL_STMT = "SELECT GIFT_NO,GIFT_NAME,GIFT_PRICE,GIFT_PIC FROM GIFT WHERE GIFT_IS_ON LIKE '%上架中%' ORDER BY GIFT_NO";
	private static final String GET_PIC_STMT = "SELECT GIFT_PIC FROM　GIFT WHERE GIFT_NO=?";
	private static final String GET_ALL = "SELECT G.GIFT_NO,G.GIFT_NAME,G.GIFT_PRICE,G.GIFT_PIC,GL.GIFTL_NAME FROM GIFT G "
			+ "JOIN GIFT_LABEL_DETAIL GLD ON G.GIFT_NO = GLD.GIFT_NO "
			+ "JOIN GIFT_LABEL GL ON GL.GIFTL_NO = GLD.GIFTL_NO " + "WHERE GIFT_IS_ON LIKE '%上架中%' ORDER BY GIFT_NO";

	private static final String GET_KEYWORD = "SELECT G.GIFT_NO,G.GIFT_NAME,G.GIFT_PRICE,G.GIFT_PIC,GL.GIFTL_NAME FROM GIFT G "
			+ "JOIN GIFT_LABEL_DETAIL GLD ON G.GIFT_NO = GLD.GIFT_NO "
			+ "JOIN GIFT_LABEL GL ON GL.GIFTL_NO = GLD.GIFTL_NO "
			+ "WHERE G.GIFT_NO=? AND GIFT_IS_ON LIKE '%上架中%'";
	...
	public void delete(String gift_no) {
		 ...
		 * ------------------------------------------* 1.
		 * 刪除禮物標籤明細[GIFT_LABEL_LIST] * 2. 刪除禮物[GIFT] * * * * * * * * * * * * * *
		 * * * * * * * * *
		 */
	...
	public Set<String> getByKeyWord(String keyword) {
		...
		try {
			con = ds.getConnection();
			String sql = "SELECT G.GIFT_NO FROM GIFT G " + "JOIN GIFT_LABEL_DETAIL GLD ON G.GIFT_NO = GLD.GIFT_NO "
					+ "JOIN GIFT_LABEL GL ON GL.GIFTL_NO = GLD.GIFTL_NO " + "WHERE GL.GIFTL_NAME LIKE '%" + keyword
					+ "%' OR G.GIFT_NAME LIKE '%" + keyword + "%'";
			pstmt = con.prepareStatement(sql);
			rs = pstmt.executeQuery();
			while (rs.next()) {
				list.add(rs.getString("gift_no"));
			}
